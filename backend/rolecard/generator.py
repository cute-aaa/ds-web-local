...
def generate_role_card() -> str:
    ...
    lines.append('start:{"name":"mcp__windows-mcp__FileSystem","arguments":{"mode":"write","path":"G:/Download/data.txt","content":"new line","append":true}}end')
    lines.append("```")
    ...
    lines.append("- write_file 工具只支持覆盖写入（参数：path, content），不支持 append 参数；")
    lines.append("  追加内容请使用 mcp__windows-mcp__FileSystem 工具（mode='write', append=true）。")
    lines.append("")
    lines.append("## 可用工具清单")
